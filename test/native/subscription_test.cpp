...
class RecordingBackend final : public ControllerBackend {
 public:
  ...
  BackendResult CancelSubscription(const std::string &id,
    ...
    ++cancel_calls;
    return {id == kSubscriptionId && generation == 1,
            id == kSubscriptionId && generation == 1 ? "" : "invalid_subscription"};
  }
...
void ProtocolEstablishesBeforeDeliveryAndRetires() {
  ...
  auto cancelled = protocol.ProcessLine(
      R"({"version":1,"id":"3","operation":"unsubscribe","parameters":{"subscription_id":"abcdef0123456789abcdef0123456789","generation":1},"timeout_ms":1000})");
  assert(cancelled.keep_running && cancelled.frame->find(R"("ok":true)") !=
         std::string::npos);
  assert(asynchronous.size() == 3);
